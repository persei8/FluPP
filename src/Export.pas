...
uses Classes, SysUtils, Math, Grids, LCLIntf, LCLType, LMessages, DOM, XMLRead, Dialogs;
...
procedure writeValStrings(XMLElem: TXMLDocument; StringList: TStrings);
var i: Word;
begin
  if StringList.Count > 0 then
    for i := 0 to StringList.Count - 1 do
      //TODO XMLElem.Properties.Add(StringList.Names[i], StringList.ValueFromIndex[i]);
end;

procedure writeObjStrings(XMLElem: TDOMNode; ItemName: String; var StringList: TStrings);
var XMLSubElem: TDOMNode;
    i: Word;
begin
  //if StringList.Count > 0 then
...
procedure writeFlight(XMLElem: TDOMNode; GridIdx, Row: Word);
var Col: Word;
begin
...
procedure writeColumns(XMLElem: TDOMNode; GridIdx: Word);
var XMLSubElem: TDOMNode;
    i: Word;
begin
...
begin
  ...
  try
   ...
   OpenDocument(PChar(FLuPPDomain+'/gmaps/'+Params)); { *Converted from ShellExecute* }
end;
...
procedure ExportGoogleEarth;
{----------}
  procedure AddPoint(Airport: TAirport; XMLElem: TDOMNode);
  begin
...
{----------}
  procedure AddLine(Airport1, Airport2: TAirport; XMLElem: TDOMNode);
  var SubElem: TDOMNode;
  begin
...
{----------}
var
  DocElem, FolElem, PlaceMElem, StyleElem, StyleSubElem: TDOMNode;
  GridRect: TGridRect;
  ViaTmp: string;
  Row: Word;
...
begin
  ...
  //   OpenDocument(PChar(ExtractFileName(FluFileName)+'.kml')); { *Converted from ShellExecute* }
  //finally
  //  Free;
  //  DecimalSeparator := TmpDecimalSeparator;
  //end;
