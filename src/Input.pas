...
begin
  ...
  GridVia.Cells[0,0]  := ('Nr.');
  GridVia.Cells[1,0]  := ('Airport');
  GridVia.Cells[2,0]  := ('Distance');

  GridKatTime.ColWidths[0] := -1;
  GridKatTime.ColWidths[1] := 90;
  GridKatTime.ColWidths[2] := 60;
  GridKatTime.Cells[1,0]  := ('Category');
  GridKatTime.Cells[2,0]  := ('Time');

  GridContest.ColWidths[0] := -1;
  GridContest.ColWidths[1] := 80;
  GridContest.ColWidths[2] := 45;
  GridContest.ColWidths[3] := 45;
  GridContest.Cells[1,0]  := ('Category');
  GridContest.Cells[2,0]  := ('Distance');
  GridContest.Cells[3,0]  := ('Points');
...
begin
  ...
  begin
    RaiseError(('Please enter aircraft id'), CBAircraftID);
    Exit;
  end; 
  if CBAircraftType.Text = '' then
  begin
    RaiseError(('Please enter aircraft type'), CBAircraftType);
    Exit;
  end;
  ...
  begin
    ...
    begin
      RaiseError(('Please enter start time'), MEFlightTimeDep);
      Exit;
    end;
    if MEFlightTimeArr.Text = '  :  ' then
    begin
      RaiseError(('Please enter landing time'), MEFlightTimeArr);
      Exit;
    end;
  end;
  ...
  begin
    ...
    begin
      RaiseError(('Please enter start time'), MEBlockTimeDep);
      Exit;
    end;
    if MEBlockTimeArr.Text = '  :  ' then
    begin
      RaiseError(('Please enter landing time'), MEBlockTimeArr);
      Exit;
    end;
  end;
  if CBAPFrom.Text = '' then
  begin
    RaiseError(('Please enter departure location'), CBAPFrom);
    Exit;
  end;
  if CBAPTo.Text = '' then
  begin
    RaiseError(('Please enter arrival location'), CBAPTo);
    Exit;
  end;
end;
...
begin
  ...
  begin
    MessageDlg(('Aircraft id already exists!'),mtWarning,[mbOk],0);
    Exit;
  end;
  DataChanged := True;
  FMain.StatusBar1.Panels[2].Text := '*';
...
begin
  ...
  begin
    MessageDlg(('Person already exists!'),mtWarning,[mbOk],0);
    Exit;
  end;
...
begin
  ...
  begin
    ...
    begin
      MessageDlg(('Please enter a time'),mtWarning,[mbOK],0);
...
begin
  ...
  begin
    MessageDlg(('Airport aleready exists!'),mtWarning,[mbOk],0);
    Exit;
  end;
...
var
  InputStr: String;
begin
  if InputQuery(('Please enter a category'),('Please enter a category'),InputStr) then
  begin
    if GridActiveChild.ACCategories.IndexOf(InputStr) >= 0 then
    begin
      MessageDlg(('Category already exists!'),mtWarning,[mbOk],0);
      Exit;
    end;
...
begin
  GridVia.Cells[0,1]  := ('From');
  GridVia.Cells[0,2]  := ('To');
  GridVia.Cells[1,1] := '';
  GridVia.Cells[1,2] := '';
  GridVia.Cells[2,2] := '';
  GridVia.RowCount := 3;
...
begin
  ...
  try
    ...
    InputBox.Caption := ('Please enter start time');
    InputBox.Width := 250;
    InputBox.Height := 150;

    EditTime := TMaskEdit.Create(InputBox);
    EditTime.Parent := InputBox;
    EditTime.EditMask := '!90:00;1;';
    EditTime.Width := 70;
    EditTime.Left := 80;
    EditTime.Top := 30;
    ...
    repeat InputBox.ShowModal;
      ...
      begin
        try
          ...
          MessageDlg(('Please enter a valid time'),mtWarning,[mbOK],0);
...
begin
  ...
  //    begin
  //      MessageDlg(format(('File ''%s'' could not be copied'),[FileQueue.Strings[i]]),mtError,[mbOK],0);
  //      Exit;
  //    end;
  //  end;
  //end;
...
begin
  ...
  begin
    ...
    EditPoints.EditLabel.Caption := ('Points');
...
begin
  if CAVOK.Checked then begin
    Metar.Text := 'CAVOK/';
    WeatherText.Text := ('Ceiling and Visibility OK');
  end else begin
    ...
    if DESCRIPTOR.ItemIndex > 0 then begin
      if INTENSITY.ItemIndex = 0 then WeatherText.Text := ('Moderate')+ ' ';
      if INTENSITY.ItemIndex = 1 then WeatherText.Text := ('Light')+ ' ';
      if INTENSITY.ItemIndex = 2 then WeatherText.Text := ('Heavy')+ ' ';
    end;

    { ------------------------------------------------------------------------ }

    if DESCRIPTOR.ItemIndex = 1 then WeatherText.Text := WeatherText.Text + ('shallow');
    if DESCRIPTOR.ItemIndex = 2 then WeatherText.Text := WeatherText.Text + ('patches');
    if DESCRIPTOR.ItemIndex = 3 then WeatherText.Text := WeatherText.Text + ('partial');
    if DESCRIPTOR.ItemIndex = 4 then WeatherText.Text := WeatherText.Text + ('low drifting');
    if DESCRIPTOR.ItemIndex = 5 then WeatherText.Text := WeatherText.Text + ('blowing');
    if DESCRIPTOR.ItemIndex = 6 then WeatherText.Text := WeatherText.Text + ('shower(s)');
    if DESCRIPTOR.ItemIndex = 7 then WeatherText.Text := WeatherText.Text + ('thunderstorm');
    if DESCRIPTOR.ItemIndex = 8 then WeatherText.Text := WeatherText.Text + ('freezing');

    { ------------------------------------------------------------------------ }

    if ((PRECIPITATION.ItemIndex <= 0) and (DESCRIPTOR.ItemIndex = 2)) then WeatherText.Text := WeatherText.Text + ',';
    if ((PRECIPITATION.ItemIndex > 0) and (DESCRIPTOR.ItemIndex = 2)) then WeatherText.Text := WeatherText.Text + (' of');

    if ((PRECIPITATION.ItemIndex <= 0) and (DESCRIPTOR.ItemIndex = 6)) then WeatherText.Text := WeatherText.Text + ',';
    if ((PRECIPITATION.ItemIndex > 0) and (DESCRIPTOR.ItemIndex = 6)) then WeatherText.Text := WeatherText.Text + (' of');

    if ((PRECIPITATION.ItemIndex <= 0) and (DESCRIPTOR.ItemIndex = 7)) then WeatherText.Text := WeatherText.Text + ',';
    if ((PRECIPITATION.ItemIndex > 0) and (DESCRIPTOR.ItemIndex = 7)) then WeatherText.Text := WeatherText.Text + (' and');

    if PRECIPITATION.ItemIndex = 1 then WeatherText.Text := WeatherText.Text + ' ' + ('drizzle');
    if PRECIPITATION.ItemIndex = 2 then WeatherText.Text := WeatherText.Text + ' ' + ('rain');
    if PRECIPITATION.ItemIndex = 3 then WeatherText.Text := WeatherText.Text + ' ' + ('snow');
    if PRECIPITATION.ItemIndex = 4 then WeatherText.Text := WeatherText.Text + ' ' + ('snow grains');
    if PRECIPITATION.ItemIndex = 5 then WeatherText.Text := WeatherText.Text + ' ' + ('ice crystals');
    if PRECIPITATION.ItemIndex = 6 then WeatherText.Text := WeatherText.Text + ' ' + ('ice pellets');
    if PRECIPITATION.ItemIndex = 7 then WeatherText.Text := WeatherText.Text + ' ' + ('hail');
    if PRECIPITATION.ItemIndex = 8 then WeatherText.Text := WeatherText.Text + ' ' + ('small hail and/or snow pellets');

    { ------------------------------------------------------------------------ }

    if ((OBSCURATION1.ItemIndex > 0) and (PRECIPITATION.ItemIndex > 0)) then WeatherText.Text := WeatherText.Text + (' and');
    if OBSCURATION1.ItemIndex = 1 then WeatherText.Text := WeatherText.Text + ' ' + ('fog');
    if OBSCURATION1.ItemIndex = 2 then WeatherText.Text := WeatherText.Text + ' ' + ('mist');

    { ------------------------------------------------------------------------ }

    if ((OBSCURATION2.ItemIndex > 0) and (OBSCURATION1.ItemIndex > 0)) or
       ((OBSCURATION2.ItemIndex > 0) and (PRECIPITATION.ItemIndex > 0)) then WeatherText.Text := WeatherText.Text + (' and');
    if OBSCURATION2.ItemIndex = 1 then WeatherText.Text := WeatherText.Text + ' ' + ('smoke');
    if OBSCURATION2.ItemIndex = 2 then WeatherText.Text := WeatherText.Text + ' ' + ('volcanic ash');
    if OBSCURATION2.ItemIndex = 3 then WeatherText.Text := WeatherText.Text + ' ' + ('widespread dust');
    if OBSCURATION2.ItemIndex = 4 then WeatherText.Text := WeatherText.Text + ' ' + ('sand');
    if OBSCURATION2.ItemIndex = 5 then WeatherText.Text := WeatherText.Text + ' ' + ('haze');

    { ------------------------------------------------------------------------ }

    if ((OTHER.ItemIndex > 0) and (OBSCURATION1.ItemIndex > 0)) or
       ((OTHER.ItemIndex > 0) and (OBSCURATION2.ItemIndex > 0)) or
       ((OTHER.ItemIndex > 0) and (PRECIPITATION.ItemIndex > 0)) then WeatherText.Text := WeatherText.Text + (' and');
    if OTHER.ItemIndex = 1 then WeatherText.Text := WeatherText.Text + ' ' + ('dust/sand whirls');
    if OTHER.ItemIndex = 2 then WeatherText.Text := WeatherText.Text + ' ' + ('squalls');
    if OTHER.ItemIndex = 3 then WeatherText.Text := WeatherText.Text + ' ' + ('funnel cloud(s) (tornado or waterspout)');
    if OTHER.ItemIndex = 4 then WeatherText.Text := WeatherText.Text + ' ' + ('sandstorm');
    if OTHER.ItemIndex = 5 then WeatherText.Text := WeatherText.Text + ' ' + ('duststorm');
    ...
    if CLOUDS.ItemIndex <= 0 then begin
      ...
      WeatherText.Text := WeatherText.Text + ('Clear sky, no clouds.');
      CLOUDBASE.Enabled := false;
      CLOUDTYPE.Enabled := false;
      exit;
    end else begin
      if WeatherText.Text > '' then WeatherText.Text := WeatherText.Text + #13+#10;
      if CLOUDS.ItemIndex = 1 then WeatherText.Text := WeatherText.Text + (Format('Cloudy (1/8 - 2/8) in %s feet', [CLOUDBASE.Text]));
      if CLOUDS.ItemIndex = 2 then WeatherText.Text := WeatherText.Text + (Format('Cloudy (3/8 - 4/8) in %s feet', [CLOUDBASE.Text]));
      if CLOUDS.ItemIndex = 3 then WeatherText.Text := WeatherText.Text + (Format('Cloudy (5/8 - 7/8) in %s feet', [CLOUDBASE.Text]));
      if CLOUDS.ItemIndex = 4 then WeatherText.Text := WeatherText.Text + (Format('Overcast in %s feet', [CLOUDBASE.Text]));
