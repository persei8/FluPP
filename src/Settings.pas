...
type
  ...
  private
    ...
    ACTimeCat_S,
    ACContestCat_S,
    Settings_S: Array of TStringList;
    LicenseCategories_S,
    LicenseTimeCat_S,
    LicenseDates_S,
    AccLicenses_S,
    OptConditions_S: Array of TStringList;
    Medicals_S: TStringList;
...
begin
  ...
  begin
    ...
    if GridChild(j).Settings.Values['DistUnit'] <> Settings_S[j].Values['DistUnit'] then
      if MessageDlg(('Do you want to convert the distances into the new unit?'),mtConfirmation,[mbYes,mbNo],0) = mrYes then
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        MessageDlg(('Passengert does already exists!'),mtWarning,[mbOk],0);
        Exit;
      end;
    ...
    begin
      ...
      begin
        MessageDlg(('Airport already exists!'),mtWarning,[mbOk],0);
        Exit;
      end;
  ...
  begin
    MessageDlg(format(('Please enter ''%s''!'),[LabelAC.Caption]),mtWarning,[mbOk],0);
    FSettings.ActiveControl := EditAC;
  end
  else if (EditTyp.Text = '') then
  begin
    MessageDlg(('Please enter Type!'),mtWarning,[mbOk],0);
...
var
  i, j: Word;
  idx: Integer;
  MedList, LicFileList: TStringList;
begin
...
begin
  ...
  try
    ...
    InputBox.Caption := ('Categories for Aircraft: '+AircraftList.Items[AircraftList.ItemIndex]);
    InputBox.Width := 250;
    InputBox.Height := 250;
